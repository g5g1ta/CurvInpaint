...
function Home() {
  ...
  // Use effect hook to generate the matrix effect once when the component mounts
  useEffect(() => {
  ...
  const prevImage = () => {
    if (currentCategory) {
      setCurrentIndex((prevIndex) => (prevIndex > 0 ? prevIndex - 1 : images[currentCategory].length - 1));
    }
  };

  const nextImage = () => {
    if (currentCategory) {
      setCurrentIndex((prevIndex) => (prevIndex < images[currentCategory].length - 1 ? prevIndex + 1 : 0));
    }
  };
  ...
  return (
    <div className="home-container">
      ...
      <div className="nav-buttons">
        <button onClick={() => scrollToSection('stable-diffusion')}>Try Stable Diffusion</button>
        <button onClick={() => scrollToSection('inpainting')}>Try Inpainting</button>
      </div>
